...
add_filter( 'plugins_api', array('WP_Email_Template_Upgrade', 'is_external'), 11, 3 );

add_action( 'admin_menu', array( 'WP_Email_Template_Upgrade', 'auto_check_get_version_info' ) );
...
class WP_Email_Template_Upgrade
{
		...
		if(!$respone_api){
			...
			if(is_array($version_info)){
				if ($version_info[1] == 'unvalid') {
					...
					$number_failures = get_option( 'a3rev_wp_email_template_number_failures', 0 );
					if ( $number_failures >= 10 ) {
						...
						update_option( 'a3rev_wp_email_template_number_failures', $number_failures );
						set_transient("a3rev_wp_email_template_update_info", $respone_api, 7200); //change caching for 2 hours
					}
				} else {
					delete_option( 'a3rev_wp_email_template_number_failures' );
				}
			}
		}
    ...
    public static function get_changelog(){
        ...
        if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code']){
            $page_text = __('Error: ', 'wp_email_template').' '.$raw_response->get_error_message();
        }else{
	...
	public static function is_external( $external, $action, $args ) {
		if ( 'plugin_information' == $action ) {
			if ( is_object( $args ) && isset( $args->slug ) &&  get_option('a3rev_wp_email_template_plugin') == $args->slug ) {
				global $wp_version;
				$external = array(
					'tested'  => $wp_version
				);
