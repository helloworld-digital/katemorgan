        ...
        ?>
                            <div class="<?php echo $active; ?> item" style='background-image: url(<?php the_sub_field('slide_image'); ?>); background-position: top center;'>
                                ...
                                <div class="container" style="width: 67%; max-width: 1170px;">
                                    <div class="row">
                                        <div class="col-md-6">
                                            <p style="height: 350px;display: table-cell;vertical-align: middle;font-size: 40px;text-align: center;color: white;line-height: 50px;">
                                                <?php the_sub_field('slide_text'); $x++; ?>
                                            </p>
                                        </div>
...
foreach ($all_categories as $cat) {
    ...
    if($cat->category_parent == 0) {
        ...
        if ($cat->name=="Packs"){
        	echo '<label class="onsale">From just $'.money_format('%i', $lowest_price).' a pack</label>';
        }

        else if($cat->name=="Essentials"){
        	echo '<label class="onsale">From just $'.money_format('%i', $lowest_price).'</label>';
        }
