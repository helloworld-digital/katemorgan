 ...
 * header menu (should you choose to use one)
 */
function dazzling_secondary_header_menu() {
  // display the WordPress Custom Menu if available
  wp_nav_menu(array(
    'menu'              => 'secondary',
...
function add_search_box_to_menu( $items, $args ) {
    ...
    if( $args->theme_location == 'secondary' )
        return $items."<li class='menu-header-search'><form action='".get_home_url()."' id='searchform' method='get'><i class='fa fa-search'></i><input class='searchbox' type='text' name='s' id='s' placeholder='Search'></form></li>";

    return $items;
}


add_filter('wp_nav_menu_items','add_join_button_to_menu', 10, 2);
function add_join_button_to_menu( $items, $args ) {
 
    if( $args->theme_location == 'primary' && is_user_logged_in() === false){
        return $items."<li class='menu-header-button'><a href='".get_home_url()."/register'><button>Join Free</button></a></li>";
    }
